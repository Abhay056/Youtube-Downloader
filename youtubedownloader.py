...
def video_screen():
    ...
    layout = [  [sg.Text('Youtube Video Downloader')],
            [sg.Text('Enter Link ->'), sg.InputText()],
            [sg.Button('Ok'), sg.Button('Cancel')],
            [sg.Checkbox('Want Subtitles ?')]]
    ...
    while True:
        ...
        if event == sg.WIN_CLOSED or event == 'Cancel': # if user closes window or clicks cancel
            ...
            url = data[0]
            sub = data[1]
            download_video(sub, url)

    window.close()

def download_video(sub,url):
    ydl_opts = {'writesubtitles': sub, 'skip-download': True}
